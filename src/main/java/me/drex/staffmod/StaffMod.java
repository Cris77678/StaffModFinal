...
public class StaffMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        // Evento de chat: mute, staff-chat, anti-spam
        ServerMessageEvents.ALLOW_CHAT_MESSAGE.register((message, sender, params) -> {
            PlayerData pd = DataStore.get(sender.getUUID());
            if (pd != null && pd.isMuteActive()) {
                sender.sendSystemMessage(Component.literal(
                    "§c[sᴛᴀꜰꜰ] Estás silenciado. Expira: §e" + PlayerData.formatExpiry(pd.muteExpiry)));
                return false;
            }
        ...
        ServerLifecycleEvents.SERVER_STARTED.register(server -> {
            ...
            // Tarea de expiración de castigos cada 10 segundos (async, con bridge al main thread)
            StaffModAsync.scheduleAsync(new ExpirationTask(server), 10, 10, TimeUnit.SECONDS);

            // Alertas de Cobblemon (opcional)
            try {
        ...
        // Tick del servidor: anuncios automáticos
